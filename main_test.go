...
func TestGetProfileFromRequest(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			profile, isFallback := GetProfileFromRequest(req, DefaultProfile)
...
func TestTransportCache(t *testing.T) {
	...
	cache := NewTransportCache(100*time.Millisecond, 10, false)
	...
	t.Run("evicts expired entries", func(t *testing.T) {
		...
		cacheEvict := NewTransportCache(cacheTTL, 10, false)
	...
	t.Run("respects max entries limit", func(t *testing.T) {
		cacheLimited := NewTransportCache(time.Hour, 2, false)
	...
	t.Run("caches by profile and proxy combination", func(t *testing.T) {
		cacheWithProxy := NewTransportCache(time.Hour, 10, false)

		// Same profile, different proxy = different transport
		transport1, _ := cacheWithProxy.GetOrCreate("chrome_133", "socks5://proxy1:1080")
		transport2, _ := cacheWithProxy.GetOrCreate("chrome_133", "socks5://proxy2:1080")

		if transport1 == transport2 {
			t.Error("expected different transport instances for different proxy URLs")
		}

		// Same profile, same proxy = same transport
		transport3, _ := cacheWithProxy.GetOrCreate("chrome_133", "socks5://proxy1:1080")

		if transport1 != transport3 {
			t.Error("expected same transport instance for same profile and proxy")
		}
	})
}
...
func TestTransportCacheCloseIdleConnections(t *testing.T) {
	cache := NewTransportCache(time.Hour, 10, false)
...
func TestFingerprintHeaderRouting(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.header, func(t *testing.T) {
			...
			profile, _ := GetProfileFromRequest(req, DefaultProfile)
...
func TestNewTransportCache(t *testing.T) {
	cache := NewTransportCache(time.Minute, 50, false)
	...
	if cache.insecureSkipVerify != false {
...
// --- TestGetProxyFromRequest ---

func TestGetProxyFromRequest(t *testing.T) {
	tests := []struct {
		name        string
		headers     map[string]string
		expectedURL string
	}{
		{name: "X-FP-Proxy header with socks5 URL", headers: map[string]string{"X-FP-Proxy": "socks5://user:pass@proxy:1080"}, expectedURL: "socks5://user:pass@proxy:1080"},
		{name: "X-FP-Proxy header with http URL", headers: map[string]string{"X-FP-Proxy": "http://user:pass@proxy:8080"}, expectedURL: "http://user:pass@proxy:8080"},
		{name: "X-FP-Proxy header with spaces", headers: map[string]string{"X-FP-Proxy": "  socks5://proxy:1080  "}, expectedURL: "socks5://proxy:1080"},
		{name: "No X-FP-Proxy header", headers: map[string]string{}, expectedURL: ""},
	}
